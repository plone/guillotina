...
async def run():
    ...
    for idx in range(args.segments - 1):
        ...
        keys.append(
            (await conn.fetch(f"select zoid from objects order by zoid limit 1 offset {offset};"))[
                0
            ][0]
        )
        ids.append(
            (await conn.fetch(f"select id from objects order by id limit 1 offset {offset};"))[0][0]
        )

    print(f"splitting on keys: {keys}")
    sql = "ALTER TABLE objects SPLIT AT VALUES {};".format(", ".join([f"('{k}')" for k in keys]))
