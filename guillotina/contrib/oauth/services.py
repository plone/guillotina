...
from guillotina.response import HTTPBadRequest, HTTPFound, HTTPPreconditionFailed, HTTPUnauthorized, Response
...
class OAuthAuthorizationServerMetadata(OAuthService):
...
class OAuthOpenIDMetadata(OAuthAuthorizationServerMetadata):
    pass
...
class OAuthRegister(OAuthService):
    async def __call__(self):
        ...
        return {key: client[key] for key in (
            "client_id",
            "client_name",
            "redirect_uris",
            "grant_types",
...
class OAuthAuthorize(OAuthService):
    async def __call__(self):
        storage = await self.storage()
        ...
        if self.request.method == "POST":
            ...
            if "application/json" in content_type:
                ...
                data = parse_form_encoded(await self.request.text())
            params.update(data)
        client = storage["clients"].get(params.get("client_id"))
        if client is None:
            return _html("Unknown OAuth client", status=400)
        redirect_uri = params.get("redirect_uri")
        redirect_valid = redirect_uri in client["redirect_uris"]
        ...
        if not params.get("code_challenge"):
            return self._redirect_error(redirect_uri, "invalid_request", params.get("state"))
        if params.get("code_challenge_method") != "S256":
            return self._redirect_error(redirect_uri, "invalid_request", params.get("state"))
        scopes = normalize_list(params.get("scope"))
        resources = validate_resource(self.request, self.context, params.get("resource"))
        user = get_authenticated_user()
        if user is None or getattr(user, "id", "Anonymous User") == "Anonymous User":
            if self.request.method == "POST" and params.get("username"):
                user = await _authenticate_basic(params.get("username"), params.get("password", ""))
                if user is None:
                    return _html("Login failed", status=401)
            else:
                return _html("<form method='post'>Username <input name='username'>Password "
        ...
        if ckey not in storage["consents"] and params.get("decision") != "allow":
            if params.get("decision") == "deny":
                return self._redirect_error(redirect_uri, "access_denied", params.get("state"))
            return _html(
        ...
        register_changed(storage)
        raise HTTPFound(redirect_with_params(redirect_uri, {"code": raw_code, "state": params.get("state")}))

    def _redirect_error(self, redirect_uri, error, state):
        raise HTTPFound(redirect_with_params(redirect_uri, {"error": error, "state": state}))
...
class OAuthToken(OAuthService):
    async def __call__(self):
        storage = await self.storage()
        data = parse_form_encoded(await self.request.text())
        grant_type = data.get("grant_type")
        if grant_type == "authorization_code":
            return self._authorization_code(storage, data)
        if grant_type == "refresh_token":
            return self._refresh_token(storage, data)
        raise HTTPBadRequest(content={"error": "unsupported_grant_type"})

    def _authorization_code(self, storage, data):
        ...
        access_token, _claims = issue_access_token(
            issuer=container_url(self.request, self.context),
    ...
    def _refresh_token(self, storage, data):
...
class OAuthRevoke(OAuthService):
    async def __call__(self):
        storage = await self.storage()
        data = parse_form_encoded(await self.request.text())
