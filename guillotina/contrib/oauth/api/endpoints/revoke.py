...
from guillotina.contrib.oauth.api.endpoints.common import REVOCATION_REQUEST_SINGLETON_PARAMS
...
async def token_revocation_endpoint(service, store):
    ...
    try:
        data = parse_form_encoded(
            await service.request.text(), singleton_fields=REVOCATION_REQUEST_SINGLETON_PARAMS
        )
    ...
    if await rate_limit_exceeded(
        f"oauth-revoke:{peer_ip_address(service.request)}",
