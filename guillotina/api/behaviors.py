...
@configure.service(
    ...
    responses={
        ...
        "412": {"description": "Behavior already assigned here"},
    },
)
async def default_patch(context, request):
    ...
    if behavior_class is None:
        return Response(content={"reason": "Could not find behavior"}, status=404)
...
@configure.service(
    ...
    responses={
        ...
        "412": {"description": "Behavior not assigned here"},
    },
)
...
async def default_get(context, request):
    ...
    for name, utility in get_utilities_for(IBehavior):
        ...
        if name not in result["dynamic"] and name not in result["static"]:
            adaptable = query_adapter(context, utility.interface, name="", default=None)
            if adaptable:
