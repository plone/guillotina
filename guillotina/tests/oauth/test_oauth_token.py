...
async def test_code_token_and_refresh_rotation(container_install_requester):
    async with container_install_requester as requester:
        ...
        _response, status = await requester(
            ...
            data=f"grant_type=authorization_code&client_id={client['client_id']}&redirect_uri={client['redirect_uris'][0]}&code={code}&code_verifier={verifier}",
        )
        assert status == 400
...
async def test_token_rejects_bad_pkce_and_redirect(container_install_requester):
    async with container_install_requester as requester:
        ...
        _response, status = await requester(
            "POST",
            "/db/guillotina/oauth/token",
            data=f"grant_type=authorization_code&client_id={client['client_id']}&redirect_uri={client['redirect_uris'][0]}&code={code}&code_verifier=bad",
        )
        assert status == 400
        code, verifier = await authorize_code(
            requester, client, scope="guillotina:mcp.read guillotina:mcp.search"
        )
        _response, status = await requester(
            "POST",
            "/db/guillotina/oauth/token",
            data=f"grant_type=authorization_code&client_id={client['client_id']}&redirect_uri=http://127.0.0.1:9999/cb&code={code}&code_verifier={verifier}",
        )
        assert status == 400
