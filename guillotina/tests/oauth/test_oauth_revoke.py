...
async def test_revoke_refresh_token(container_install_requester):
    async with container_install_requester as requester:
        ...
        _response, status = await requester("POST", "/db/guillotina/oauth/revoke", data=f"client_id={client['client_id']}&token={token['refresh_token']}&token_type_hint=refresh_token")
        assert status == 200
...
async def test_revoke_unknown_token(container_install_requester):
    async with container_install_requester as requester:
        client = await register_client(requester)
        _response, status = await requester("POST", "/db/guillotina/oauth/revoke", data=f"client_id={client['client_id']}&token=unknown")
        assert status == 200
