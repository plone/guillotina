...
async def test_set_dynamic_behavior(custom_type_container_requester):
    async with custom_type_container_requester as requester:
        ...
        response, status = await requester(
            ...
            data=json.dumps({"b": "guillotina.test_package.ITestBehavior"}),
        )
        assert status == 412
        ...
        response, status = await requester(
            "PATCH",
            "/db/guillotina/item1/@behaviors",
            data=json.dumps({"behavior": "guillotina.test_package.ITestBehavior"}),
        )
        assert status == 200
...
async def test_set_dynamic_context_behavior(custom_type_container_requester):
    async with custom_type_container_requester as requester:
        ...
        response, status = await requester(
            "PATCH",
            "/db/guillotina/item1/@behaviors",
            data=json.dumps({"behavior": "guillotina.test_package.ITestContextBehavior"}),
        )
        assert status == 200
        ...
        response, status = await requester(
            "PATCH",
            "/db/guillotina/item1",
            data=json.dumps({"guillotina.test_package.ITestContextBehavior": {"foobar": "foobar"}}),
        )
        response, status = await requester("GET", "/db/guillotina/item1")
        assert "guillotina.test_package.ITestContextBehavior" in response
...
async def test_auto_serialize_behavior(custom_type_container_requester):
    async with custom_type_container_requester as requester:
        ...
        # We create the behavior
        response, status = await requester(
            "PATCH", "/db/guillotina/item1/@behaviors", data=json.dumps({"behavior": dotted_name})
        )
        assert status == 200
        ...
        # now is should be...
        response, status = await requester("GET", f"/db/guillotina/item1?include={dotted_name}")
        assert dotted_name in response

        # set values on behavior
        response, status = await requester(
            "PATCH", "/db/guillotina/item1", data=json.dumps({dotted_name: {"foobar": "foobar"}})
        )
        response, status = await requester("GET", f"/db/guillotina/item1?include={dotted_name}")
        assert dotted_name in response
...
async def test_create_delete_dynamic_behavior(custom_type_container_requester):
    async with custom_type_container_requester as requester:
        ...
        response, status = await requester(
            "PATCH",
            "/db/guillotina/item1/@behaviors",
            data=json.dumps({"behavior": "guillotina.test_package.sldkflsdf"}),
        )
        assert status == 404
...
async def test_delete_dynamic_behavior_url(custom_type_container_requester):
    async with custom_type_container_requester as requester:
        ...
        # We delete the behavior
        response, status = await requester(
            "DELETE", "/db/guillotina/item1/@behaviors/guillotina.test_package.ITestBehavior"
        )
        assert status == 200
...
async def test_get_behaviors(custom_type_container_requester):
    async with custom_type_container_requester as requester:
        ...
        response, status = await requester("GET", "/db/guillotina/item1/@behaviors")
        assert status == 200
...
async def test_can_not_delete_concrete_behaviors(custom_type_container_requester):
    async with custom_type_container_requester as requester:
        ...
        response, status = await requester(
            "PATCH",
            "/db/guillotina/item1/@behaviors",
            data=json.dumps({"behavior": "guillotina.behaviors.dublincore.IDublinCore"}),
        )
        assert status == 412
...
async def test_add_dynamic_fields(container_requester):
    async with container_requester as requester:
            ...
            data=json.dumps(
                {
                    ...
                    IDynamicFields.__identifier__: {
                        "fields": {"foobar": {"title": "Hello field", "type": "integer"}}
                    },
                }
            ),
        )
        ...
        resp, _ = await requester(
            "GET", "/db/guillotina/foobar?include={}".format(IDynamicFields.__identifier__)
        )
        assert len(resp[IDynamicFields.__identifier__]["fields"]) == 1
        resp, status = await requester("GET", "/db/guillotina/foobar/@dynamic-fields")
        assert status == 200
...
async def test_add_dynamic_fields_values(container_requester):
    async with container_requester as requester:
            ...
            data=json.dumps(
                {
                    ...
                    IDynamicFields.__identifier__: {
                        "fields": {
                            "foobar": {"title": "Hello field", "type": "integer"},
                            "foobar_date": {"title": "Date field", "type": "date"},
                            "foobar_bool": {"title": "Bool field", "type": "boolean"},
                            "foobar_float": {"title": "Float field", "type": "float"},
                            "foobar_keyword": {"title": "Float field", "type": "keyword"},
                        }
                    },
                }
            ),
            ...
            data=json.dumps(
                {
                    ...
                    IDynamicFieldValues.__identifier__: {
                        "values": {
                            ...
                            "value": [
                                {"key": "foobar", "value": 5},
                                {"key": "foobar_date", "value": "1999/01/01"},
                                {"key": "foobar_bool", "value": False},
                                {"key": "foobar_keyword", "value": "foobar"},
                                {"key": "foobar_float", "value": 2.1},
                            ],
                        }
                    },
                }
        ...
        assert status == 204
        resp, status = await requester(
            "GET", "/db/guillotina/foobar?include={}".format(IDynamicFieldValues.__identifier__)
        )
            ...
            data=json.dumps(
                {
                    "@behaviors": [IDynamicFieldValues.__identifier__],
                    IDynamicFieldValues.__identifier__: {
                        "values": {"op": "assign", "value": {"key": "foobar", "value": 5}}
                    },
                }
            ),
        )
            ...
            data=json.dumps(
                {
                    "@behaviors": [IDynamicFieldValues.__identifier__],
                    IDynamicFieldValues.__identifier__: {
                        "values": {"op": "assign", "value": {"key": "foobar-blah", "value": "5"}}
                    },
                }
            ),
        )
            ...
            data=json.dumps(
                {
                    ...
                    "id": "foobar",
                    IDynamicFields.__identifier__: {
                        "fields": {"foobar": {"title": "Hello field", "type": "keyword"}}
                    },
                }
            ),
        )
            ...
            data=json.dumps(
                {
                    "@behaviors": [IDynamicFieldValues.__identifier__],
                    IDynamicFieldValues.__identifier__: {
                        "values": {"op": "assign", "value": {"key": "foobar", "value": "foo"}}
                    },
                }
            ),
        )
