...
Guillotina provide an imperative security system. Permissions are computed for
a given node in the resource tree using some concept we are going to describe
in this document.

## Basics

Let's do that by examples, to do so follow the steps from
...
A principal is like a tag and we grant permission to that tag. The fact is that
user's usernames are a principals, more on that later.
...
At the moment alice can users and groups which is not convenient for a
...
For example let's give to principal "Alice" `guillotina.Editor` role on 
